...
dta <-  merge(dta,sc_baseline, by.x=c("district","subcounty"),by.y=c("district","sub"))
dta$hetero_ind <- dta$connected==1
}
if (hetero == 5) {
...
dta$hetero_ind <- dta$eth_fract > 0.356
}
...
ols <- lm(as.formula(paste(paste(outcomes[i],"information*deliberation*hetero_ind+a21",sep="~"),baseline_outcomes[i],sep="+")), data=dta[dta$district_baraza == 0,]) 
...
df_ancova[,2,i] <- c(res[10,1],res[10,2],res[10,5], conf[10,4],conf[10,5], nobs(ols))
df_ancova[,3,i] <- c(res[11,1],res[11,2],res[11,5], conf[11,4],conf[11,5], nobs(ols))
...
df_ancova[,5,i] <- c(res[8,1],res[8,2],res[8,5], conf[8,4],conf[8,5], nobs(ols))
